...
def home(person_info):
    ...
    with cols[1]:
      if bmi_class == "標準":
        hc.info_card(title='BMI', content=f'{round(bmi, 2)}', sentiment='good',bar_value= round((bmi * 100) / bmi_max, 2),theme_override=hc_theme)
      else:
        hc.info_card(title='BMI', content=f'{round(bmi, 2)}', sentiment='bad',bar_value= round((bmi * 100) / bmi_max, 2),theme_override=hc_theme)
    with cols[0]:
