...
if page == "ホーム":
    ...
    with col2:
        ...
        fig.update_layout(barmode='overlay', showlegend=True, height=600, width=800)
        st.plotly_chart(fig)
    ...
    with col1:
        st.subheader("地域データ")
    with col2:
        fig_pie = px.pie(data, names='area', title='Users by Area', height=400, width=800)
        st.plotly_chart(fig_pie)

        area_counts = data['area'].value_counts()
        fig_bar = px.bar(area_counts, x=area_counts.index, y=area_counts.values, title='Measurements by Area', height=400, width=800)
    ...
    with col1:
        st.subheader("散布図行列")
    with col2:
        scatter_matrix_fig = px.scatter_matrix(data, dimensions=['height', 'weight', 'body_fat_per'], color='area', height=800, width=800)
    ...
    with col2:
        ...
        fig_box.update_layout(showlegend=False, height=900, width=800)
